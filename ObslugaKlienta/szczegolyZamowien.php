...
$komendaSql = "SELECT dbo.Zamowienie.IdKlient, IdZamowienia, dbo.Klient.IdKlient AS Nr, dbo.Klient.Imie AS ImieKlienta, OdlegloscKM, Cena, Uwagi, DataZlozenia, DataRealizacji, TreminRealizacji, Opoznienie, DataOdbioruLadunku, dbo.Pracownik.Imie AS Spedytor
FROM dbo.Zamowienie
...
WHERE dbo.Zamowienie.IdKlient=$IdKlienta;
";
...
$zbiorWierszy= sqlsrv_query($polaczenie, $komendaSql);
	if(($zbiorWierszy==null)||(sqlsrv_has_rows($zbiorWierszy)== false)){
		print("<h2> Brak Ladunku dla wybranego zamowienia</h2>
		");
	}
	else{
		/*$komendaSql= "SELECT Imie, Nazwisko, NrTelefonu, Email, Stanowisko, IdPracownika, Licencja
...
print("<h2>Wykaz ladunkow dla wybranego zamowienia </h2>");
		//Poczatek tabeli: Imie, Nazwisko, NrTelefonu, Email, Stanowisko, IdPracownika, Licencja 
		print("<table>
				<thead>
				 <tr>
